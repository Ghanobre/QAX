...
# Consulta de usuário
#     Create Session    alias=Teste1    url=https://serverest.dev/    headers=&{headers}   disable_warnings=true
#     ${response}    GET On Session    alias=Teste1    url=usuarios/${_id_suite}    expected_status=any
#     Status Should Be    200    ${response}
#     Dictionary Should Contain Item    ${response.json()}    nome    Fulano da Silva

# Alterar de usuario
#     Create Session    alias=Teste1    url=https://serverest.dev/    headers=${headers}    disable_warnings=true
#     ${response}    PUT On Session    alias=Teste1    url=usuarios/${_id_suite}    expected_status=any    json=${new_body}
#     Status Should Be    200    ${response}
#     Dictionary Should Contain Item    ${response.json()}       message    Registro alterado com sucesso

# Exclusao de usuário
#     Create Session    alias=Teste1    url=https://serverest.dev/    headers=&{headers}   disable_warnings=true
#     ${response}    DELETE On Session    alias=Teste1    url=usuarios/${_id_suite}    expected_status=any
#     Status Should Be    200    ${response}
